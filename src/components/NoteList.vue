...
<template>
    <div class="border-r-2 border-lime-800">
        ...
        <RouterLink v-for="(note, index) in notes" :to="`/notes/${note.id}`"
            ...
            <button
                class="absolute top-2 right-2 p-1 border border-orange-600 bg-orange-800 rounded cursor-pointer hidden group-hover/note:block"
                @click.stop.prevent="removeNote(index)">Remove</button>
        </RouterLink>
    </div>
</template>
